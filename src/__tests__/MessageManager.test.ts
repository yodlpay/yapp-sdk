...
describe('MessageManager', () => {
  ...
  /**
   * Tests for recoverPendingPayment method
   */
  describe('recoverPendingPayment', () => {
    it('should recover payment from URL parameters', async () => {
      const manager = new MessageManager(TEST_CONSTANTS.ORIGIN);

      // Mock URL parameters
      (global.URLSearchParams as jest.Mock).mockImplementation(() => ({
        get: (param: string) => {
          if (param === 'txHash') return TEST_CONSTANTS.TX_HASH;
          if (param === 'chainId') return TEST_CONSTANTS.CHAIN_ID.toString();
          return null;
        },
      }));

      // Mock the history.replaceState method
      mockWindow.history.replaceState = mockHistoryReplaceState;

      const result = await manager.recoverPendingPayment();
    ...
    it('should handle cancelled payment from URL parameters', async () => {
      const manager = new MessageManager(TEST_CONSTANTS.ORIGIN);

      // Mock URL parameters
      (global.URLSearchParams as jest.Mock).mockImplementation(() => ({
        get: (param: string) => {
          if (param === 'status') return 'cancelled';
          return null;
        },
      }));
      ...
      await expect(manager.recoverPendingPayment()).rejects.toThrow(
        'Payment was cancelled',
      );
    });

    it('should recover payment from session storage', async () => {
      const manager = new MessageManager(TEST_CONSTANTS.ORIGIN);

      // Spy on setupReturnUrlHandler to avoid actual implementation
      const setupReturnUrlHandlerSpy = jest.spyOn(
        manager as any,
        'setupReturnUrlHandler',
      );

      // Mock implementation to call the resolve function directly
      setupReturnUrlHandlerSpy.mockImplementation((...args: any[]) => {
        // Get the resolve callback (second argument)
        const resolve = args[1];
      ...
      // Mock the recoverPendingPayment method to return the expected result
      jest.spyOn(manager, 'recoverPendingPayment').mockResolvedValue({
        txHash: TEST_CONSTANTS.TX_HASH,
        chainId: TEST_CONSTANTS.CHAIN_ID,
      });

      // Store a payment request in session storage
      mockSessionStorage['yodl_payment_request'] = JSON.stringify({
        timestamp: Date.now(),
        memo: TEST_CONSTANTS.MEMO,
        address: TEST_CONSTANTS.ADDRESS,
      ...
      const recoveryPromise = manager.recoverPendingPayment();
    ...
    it.skip('should handle expired payment in session storage', async () => {
      const manager = new MessageManager(TEST_CONSTANTS.ORIGIN);

      // Mock session storage with an expired payment
      mockSessionStorage['yodl_payment_request'] = JSON.stringify({
        timestamp: Date.now() - 301000, // Older than 5 minutes + 1 second
        memo: TEST_CONSTANTS.MEMO,
      });

      // Create a spy for sessionStorage.removeItem
      const removeItemMock = jest.fn();
      const originalRemoveItem = global.sessionStorage.removeItem;
      global.sessionStorage.removeItem = removeItemMock;

      const result = await manager.recoverPendingPayment();

      expect(result).toBeNull();
      // Verify storage was cleaned up
      expect(removeItemMock).toHaveBeenCalledWith('yodl_payment_request');

      // Restore original method
      global.sessionStorage.removeItem = originalRemoveItem;
    });

    it.skip('should handle invalid data in session storage', async () => {
      const manager = new MessageManager(TEST_CONSTANTS.ORIGIN);

      // Mock session storage with invalid JSON
      mockSessionStorage['yodl_payment_request'] = 'invalid-json';
